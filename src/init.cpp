 ...
 * between JinyCoind, and JinyCoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/jinycoin/jinycoin>";
    const std::string URL_WEBSITE = "<https://jinycoin.com>";
...
/** Sanity checks
 *  Ensure that JinyCoin is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (strAlgo == "x16r")
         miningAlgo = ALGO_X16R; // JINY TODO: we should not be here
...
{
    // Make sure only a single JinyCoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if JinyCoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if JinyCoin is started while in a temporary directory.\n",
