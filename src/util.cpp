...
const char * const BITCOIN_CONF_FILENAME = "jiny.conf";
const char * const BITCOIN_PID_FILENAME = "jinyd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "jinyd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Jiny
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Jiny
    // Mac: ~/Library/Application Support/Jiny
    // Unix: ~/.jiny
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Jiny";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Jiny";
#else
    // Unix
    return pathRet / ".jiny";
#endif
...
const fs::path &GetBackupsDir()
{
    ...
    if (gArgs.IsArgSet("-walletbackupsdir")) {
        ...
        // JINY TODO: SetMiscWarning(strMessage);
    }
    // Default path
